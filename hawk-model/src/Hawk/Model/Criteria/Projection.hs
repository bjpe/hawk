...
asP alias p = p { as = Just alias }

instance GetSqlString Op where
  sqlString Count      = "COUNT"
  sqlString Sum        = "SUM"
  sqlString Avg        = "AVG"
  sqlString Min        = "MIN"
  sqlString Max        = "MAX"
  sqlString (Custom c) = c

instance GetSqlString Projection where
  sqlString p = case op p of
            Just o' -> sqlString o' ++ paren (column p) ++ showAs (as p)
