...
instance GetSqlString Criteria where
    sqlString c = whereClause (restriction c)
            ++ orderClause (ordering c)
            ++ extraClause (extra c)
        where
        whereClause r     = case r of
                          Nothing -> []
                          Just rs -> " WHERE " ++ sqlString rs
        orderClause o     = case o of
                          [] -> ""
                          os -> " ORDER BY " ++ intercalate ", " (map sqlString os)
        extraClause e     = ' ' : intercalate ", " e

instance GetSqlString Query where
  sqlString c =  optionsClause (option                 c)
      ...
      havingClause h    = case h of
                          Nothing -> []
                          Just hs -> " HAVING " ++ sqlString hs
      extraClause e     = ' ' : intercalate ", " e

instance GetSqlString Manipulation where
  sqlString (Delete t   c) = showDelete t   c
  sqlString (Update t s c) = showUpdate t s c
  sqlString (Insert t s  ) = showInsert t s

showDelete :: TableName -> Criteria -> String
showDelete tbl cri =
    deleteClause ++ sqlString cri
    where
        deleteClause = "DELETE FROM " ++ tbl

showUpdate :: TableName -> [(ColumnName, SqlValue)] -> Criteria -> String
showUpdate tbl s cri =
    updateClause ++ columnExpr ++ sqlString cri
    where
...
instance GetSqlValues Criteria where
    sqlValues = maybe [] sqlValues . restriction

instance GetSqlValues Query where
    sqlValues (Select _ _ _ _ h c) = sqlValues c ++ maybe [] sqlValues h

instance GetSqlValues Manipulation where
    sqlValues (Delete _ c) = sqlValues c
    sqlValues (Update _ s c) = map snd s ++ sqlValues c
    sqlValues (Insert _ s) = map snd s
